...
void tail_sbf::operator()(const blas::vector<double>& reference) throw(bad_control)
{
	...
	blas::vector<double> ned_position_error(imu->getPosition() - reference);
	blas::vector<double> ned_velocity_error(imu->getVelocity());
	...
	//Check whe feedback value??
	double heading = imu->getAngle()(2);
