...
public:
	/** @brief CvMat cast operator */
	operator CvMat() { CvMat m; m.type = (CV_MAT_MAGIC_VAL | CV_MAT_CONT_FLAG | CV_MAT_TYPE(CV_32F)); m.step = 4; m.refcount = NULL; m.hdr_refcount = 0; m.data.fl = m_vec; m.rows = 3; m.cols = 1; return m; }
...
public:
	/** @brief CvMat cast operator */
	operator CvMat() { CvMat m; m.type = (CV_MAT_MAGIC_VAL | CV_MAT_CONT_FLAG | CV_MAT_TYPE(CV_64F)); m.step = 8; m.refcount = NULL; m.hdr_refcount = 0; m.data.db = m_vec; m.rows = 3; m.cols = 1; return m; }
